...
class Constants:
    ...
    ERROR = "ERROR",
    PROCESS_NAME = "demsImportStatusPoller"
    REGION_NAME = "ca-central-1"
    AGENCY_LOOKUP_TABLE_NAME = "agency-lookups"
    HTTP_OK_CODE = 200,
    IMPORT_REQUESTED= "IMPORT-REQUESTED"

class DemsImportStatusPoller:
    """Main class to call the EDT API to get a status of the previous "import" of evidence (from the Airbyte transfer) to the particular, relevant target DEMS case."""
    ...
    def _get_ssm_parameters(self) -> dict:
        ...
        parameter_names = [
            ...
            f'/{self.env_stage}/edt/api/import_status_url',
            f'/{self.env_stage}/edt/api/import_template',
            f'/{self.env_stage}/axon/api/client_id',
            f'/{self.env_stage}/axon/api/get_list_case_evidence_url',
    ...
    def callEDTDemsApi( self, job_id, dems_case_id, dems_import_job_id)->str:
        import_id = None
        try:
            api_url = self.parameters[f'/{self.env_stage}/edt/api/import_status_url']
            api_url = api_url.replace("$$$$", dems_case_id)
            api_url = api_url.replace("####", dems_import_job_id)
            ...
            try:
                ...
                api_response = self.http.request('GET', api_url, headers=headers, body=None)
            ...
            if api_response.status < 400:
                ...
                event="DEMS Import Status API call",
                url=api_url,
                method="GET",
...
def lambda_handler(event, context):
    ...
    try:
        requester = DemsImportStatusPoller(env_stage, logger, context.aws_request_id)
