...
class EvidenceDeleter:
    def __init__(self, context: Optional[Any] = None):
        self.logger = LambdaStructuredLogger()
    ...
    def get_lambda_config(self, ssm=None) -> Dict[str, str]:
        """Get Lambda-specific configuration from SSM Parameter Store."""
        if self._lambda_config is not None:
        ...
        try:
            ...
            self.logger.log_success(event=Constants.PROCESS_NAME, message=f"Loading configuration for environment: {self.env_stage}")

            # Define SSM parameter paths based on environment
            parameter_paths = {
                'axon_bearer_token': f'/{self.env_stage}/axon/api/bearer',
                'edt_authenticate_url': f'/{self.env_stage}/edt/api/authenticate_url',
                'edt_client_id': f'/{self.env_stage}/edt/api/client_id',
                'edt_grant_type': f'/{self.env_stage}/edt/api/grant_type',
                'edt_client_secret': f'/{self.env_stage}/edt/api/client_secret',
                'edt_import_status_url': f'/{self.env_stage}/edt/api/import_status_poll_url',
                'bridge_tracking_db_connection': f'/{self.env_stage}/bridge/tracking-db/connection-string',
                'sqs-q-transfer-exception': f'/{self.env_stage}/bridge/sqs-queues/arn_q-transfer-exception',
                'sqs-q-dems-import': f'/{self.env_stage}/bridge/sqs-queues/arn_q-dems-import',
            }
            ...
            self.logger.log_ssm_parameter_collection(
            ...
            self._lambda_config = config
            self.logger.log_success(event=Constants.PROCESS_NAME, message=f"Lambda configuration loaded from SSM for environment: {self.env_stage}")
    ...
    def initialize_db_manager(self):
        ...
        if self.db_manager is None:
            self.db_manager = get_db_manager(env_param_in=self.env_stage)
            self.db_manager._initialize_pool()
            self.logger.log_success(event=Constants.PROCESS_NAME, message=f"Initialized DatabaseManager for environment: {self.env_stage}")
        else:
            print("db manager already init'd")

    def send_exception_message(self, exception_queue_url: str, job_id: str, evidence_id: str = None,
        ...
        try:
            ...
            self.logger.log_success(event=Constants.PROCESS_NAME, message=f"Sent exception message to queue for job {job_id}, evidence {evidence_id}")

        except Exception as e:
            self.logger.log_error(event=Constants.PROCESS_NAME, error=e)

    def create_sqs_message(self, job_id: str) -> Dict:
    ...
    def send_messages_to_queue(self, sqs_client, queue_url: str, messages: List[Dict], queue_type: str) -> Dict:
        ...
        for i in range(0, len(messages), Constants.SQS_BATCH_SIZE):
            ...
            try:
                ...
                for failure in failed:
                    ...
                    self.logger.log_error(event=Constants.PROCESS_NAME, error=Exception(error_msg))

                if successful:
                    self.logger.log_success(event=Constants.PROCESS_NAME, message=f"Successfully sent {len(successful)} messages to {queue_type} queue")
    ...
    def lambda_update_job_status(self, job_id: str, status_value: str, msg: str, process_name: str):
    ...
    # def update_evidence_status(self, evidence_ids: str) -> bool:
    #     job_status_code = self.db_manager.get_status_code_by_value(value=Constants.MET)
    #     try:
    ...
    #         self.logger.log_success(event=Constants.PROCESS_NAME, message=f"Successfully updated {len(evidence_ids)} evidence files")
    #         return True
    #     except Exception as e:
    #         self.logger.log_error(event=Constants.PROCESS_NAME, error=e)
    #         return False
    ...
    def handle_error_and_queue(
        ...
        try:
            ...
            self.logger.log_error(event=Constants.PROCESS_NAME, error=Exception(json.dumps(log_data)))

            # Send the error to the exception queue
            self.send_exception_message(
            ...
            self.logger.log_error(event=Constants.PROCESS_NAME, error=Exception(
                json.dumps({
    ...
    def call_edt_dems_api(self, job_id: str, dems_import_job_id: str, sqs_message_id: str, context: Any) -> bool:
        # call EDT
        ...
        try:
            edt_auth_url = self._lambda_config['edt_authenticate_url']
            payload = {
                "client_id": self._lambda_config['edt_client_id'],
                "grant_type": "client_credentials",
                "client_secret": self._lambda_config['edt_client_secret']
            }
            ...
            self.logger.log_api_call(event="call to EDT Bearer token", url=edt_auth_url, method=Constants.HTTP_METHOD_POST, status_code=response.status,
        ...
        try:
            # call to get import
            edt_get_import_url = self._lambda_config['edt_import_status_url']
            headers = {'Content-Type': 'application/x-www-form-urlencoded',
                       'Authorization': f"Bearer {edt_token}"}

            start_time = time.perf_counter()
            self.logger.log(event="EDT api url ", status=LogStatus.IN_PROGRESS, message="API URL constructed : " + edt_get_import_url)
            ...
            self.logger.log_api_call(event="call to EDT Get Import", url=edt_auth_url, method=Constants.HTTP_METHOD_GET, status_code=response.status,
                                     response_time=response_time, job_id=context.aws_request_id)
    ...
    def lambda_handler(self, event: Dict[str, Any], context: Any) -> Dict[str, Any]:
        try:
            self.logger.log_start(event="DEMS Import Status Poller Start", job_id=context.aws_request_id)
            self.base_context = {
            ...
            if not event.get("Records"):
                self.logger.log_error(event=Constants.PROCESS_NAME, error=Exception("No records found in event"))
                return {
                    'statusCode': 400,
            ...
            for record in event["Records"]:
                ...
                if not job_id or not dems_import_job_id:
                    self.logger.log_error(event=Constants.PROCESS_NAME, error=Exception("job_id and dems_import_job_id are required"))
                    return {
                        'statusCode': 400,
                        'body': json.dumps({
                ...
                if self.call_edt_dems_api(job_id, dems_import_job_id, self.base_context):
            ...
            self.logger.log_error(event=Constants.PROCESS_NAME, error=Exception(f"Lambda execution failed: {str(e)}"))
            return {
                'statusCode': 500,
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    deleter = EvidenceDeleter(context)
    deleter.get_lambda_config()  # Load config
    return deleter.lambda_handler(event, context)